...
from retrieval import NOTES_DIR, ToolBudget, ToolError, search_notes
from workflow import MockTutor, Workspace
...
class StageTwoTests(unittest.TestCase):
    ...
    def test_shared_timeout_before_and_after_tool(self):
        budget = ToolBudget()
        with patch('retrieval.time.monotonic', return_value=budget.deadline):
    ...
    def test_blocked_search_process_is_killed_at_deadline(self):
        ...
        with patch('retrieval.subprocess.run', side_effect=blocked_worker):
            with self.assertRaises(TimeoutError):
                budget.call('search_notes', {'query': '分数'})
    ...
    def test_two_searches_share_one_total_deadline(self):
        ...
        with patch('retrieval.time.monotonic', return_value=start + 1):
            with patch('retrieval.subprocess.run', return_value=subprocess.CompletedProcess([], 0, '{"results":[]}')) as run:
                budget.call('search_notes', {'query': '分数'})
                self.assertAlmostEqual(2, run.call_args.kwargs['timeout'])
        with patch('retrieval.time.monotonic', return_value=start + 2.5):
            with patch('retrieval.subprocess.run', return_value=subprocess.CompletedProcess([], 0, '{"results":[]}')) as run:
                budget.call('search_notes', {'query': '应用题'})
