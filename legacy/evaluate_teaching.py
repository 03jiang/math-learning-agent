...
from legacy.model_api import ApiTutor, HttpTransport, api_key_variable, load_model_config, request_payload
...
def frozen_sources():
    paths = sorted([*ROOT.glob('legacy/*.py'), *ROOT.glob('study/*.py'), *ROOT.glob('study/*.swift'),
...
def run_batch(output, *, mode='preview', suite='all', repeats=1, config=None, api_key='',
    ...
    if resume:
        ...
        from legacy.evaluation_scoring import create_scorecard
