...
本入口复用主页面的 `AgentStudyService`，冻结输入、模型配置、首轮请求、资料文件和调用上限。代码与离线演练已完成；**尚未执行真实请求，也没有真实工具选择成绩**。所有题目、原作答和历史记录均为自写样例，不读取日常错题本或用户照片。
...
在当前 `feat/study-smoke-audit` 分支根目录执行；真实预览应在代码提交、工作区干净后创建。输出目录必须不存在。
...
返回结果后先检查 `REPORT.md`、`responses/bNN.json` 与对应 `agent-runs/*.json`：请求次数是实际预留的 HTTP 名额；超时或中断可能已计费，会保留“完成情况未知”。诊断仅保留限长、脱敏的对外正文，不记录内部 `reasoning_content`。执行记录是私人数据，按白名单导出时排除。
