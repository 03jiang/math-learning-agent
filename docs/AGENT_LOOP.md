...
阶段 C 的程序已接入拍照解题和再次订正页面，默认关闭。已完成本机 HTTP、页面和文件持久化验证；2026-09-24首次真实联调发送2次请求，直接回答完成，工具请求解析失败，工具实际执行0次。**真实模型是否恰当选择工具、来源是否支持教学结论，尚未验收。** 不能把手写工具请求称为真实 Agent 效果。

真实验收入口支持冻结五个场景、累计最多十次模型请求、失败停止和单独确认保存。首次提前停止及离线修订见[失败复核](C_FAILURE_REVIEW.md)，运行边界见[真实联调说明](AGENT_LIVE.md)。
...
采用 DeepSeek 官方 [Tool Calls](https://api-docs.deepseek.com/guides/tool_calls/) 的 Chat Completions 回合：`assistant.tool_calls` → 程序执行 → 同 ID 的 `role=tool` 消息 → 下一次模型请求。使用 `thinking=disabled` 和 `tool_choice=auto`；名额将尽时改为 `none`。最终正文采用 `json_object`，包装为 `{"result":原分析或订正对象,"citations":[{"source_id":"来源编号","quote":"原文节选"}]}`。Agent v2 接受并检查可选 `index`，记录失败调用的限长诊断；完整真实往返仍待修订版复测。
...
文件元信息签名用于发现来源新增、修改、归档与恢复，不等同于内容可信度或语义时效性。相关性仍是关键词匹配，时间只用于相关性同分时优先较新记录；D 已加入明确的 180 天历史窗口、逐题停用、字节预算与同题追问，见 [上下文说明](CONTEXT.md)；语义上的知识过时判断及查询质量评估仍待 E。单机固定进程实现只读接口，未提供操作系统级沙箱或云端多租户隔离。
...
本轮不能勾选清单中的“真实模式演示五类工具决策”。D 的本轮要求、已确认设置、同题追问和上下文预算已接入主页面并做离线验证。后续需按最终代码重新冻结范围、确认预算后验证真实选择，再开展 E 的对照评估与人工评分。B 的旧问题与原始账本保留，修复后两条照片分析已经用户确认收藏并恢复，详见[B复测记录](B_RETEST.md)。
