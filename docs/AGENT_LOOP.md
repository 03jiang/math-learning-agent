...
真实验收入口已准备，支持冻结五个场景、累计最多十次模型请求、失败停止和单独确认保存；目前仅本机演练，见 [真实联调准备](AGENT_LIVE.md)。
...
本轮不能勾选清单中的“真实模式演示五类工具决策”。D 的本轮要求、已确认设置、同题追问和上下文预算现已接入主页面并做离线验证。后续需按最终代码重新冻结范围、确认预算后验证真实选择，再开展 E 的对照评估与人工评分。既有两次照片分析的内容缺陷、未收藏状态和 B 阶段原始账本保持不变。
