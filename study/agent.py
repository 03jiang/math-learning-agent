...
from study.agent_protocol import AgentError, VERSION, parse_turn, result_message, validate_final, normalize_final
...
from study.output_contract import final_json_instructions, strict_final_payload, FINAL_FUNCTION
...
def build_agent_payload(config, instructions, context, history_enabled, image=None, work_image=None, *, limits=None, operation='analyze', output_mode='json_object'):
    bounds=limits or Limits()
    limit_text=f'\n本轮实际上限为 {bounds.model_requests} 次模型请求和 {bounds.tool_attempts} 次工具请求，以上实际上限优先。'
    if output_mode not in ('json_object','strict_tool'): raise AgentError('invalid_output_mode')
    suffix=final_json_instructions(operation,context=context) if output_mode=='json_object' else ''
    payload=build_payload(config,instructions+INSTRUCTIONS+limit_text+suffix,context,image,work_image)
    if output_mode=='strict_tool':
        return strict_final_payload(payload,operation,context=context,read_tools=definitions(history_enabled))
...
class AgentStudyService(StudyService):
    def __init__(self, config, key, *, scope, audit_dir, transport=None, limits=None, run_id=None, output_mode='json_object'):
        super().__init__(config,key,transport,output_mode=output_mode)
    ...
    def call(self, operation, instructions, context, image=None, work_image=None):
        ...
        payload=build_agent_payload(self.config,instructions,context,self.scope.history_enabled,image,work_image,limits=self.limits,operation=operation,output_mode=self.output_mode)
        ...
        trace={'schema_version':1,'run_id':self.run_id,'agent_version':VERSION,'code_commit':snapshot['commit'],
            ...
            'operation':operation,'mode':self.transport.kind,'output_mode':self.output_mode,'status':'running','started_at':stamp(),
        ...
        try:
            while len(trace['model_calls'])<self.limits.model_requests:
                ...
                final_only=(len(trace['tool_calls'])>=self.limits.tool_attempts or len(trace['model_calls'])==self.limits.model_requests-1)
                if final_only:
                    if self.output_mode=='strict_tool':
                        payload['tools']=[t for t in payload['tools'] if t['function']['name']==FINAL_FUNCTION]
                        payload['tool_choice']={'type':'function','function':{'name':FINAL_FUNCTION}}
                ...
                try:
                    ...
                    turn=parse_turn(envelope,record,seen_ids,
                                    final_function=FINAL_FUNCTION if self.output_mode=='strict_tool' else None)
                ...
                if self.scope.signature()!=signature: raise AgentError('sources_changed')
                if turn['kind']=='final':
                    candidate,trace['output_normalization']=normalize_final(turn['value'],operation)
                    value=validate_final(candidate,trace['sources'],valid_result)
                    trace['raw_final_valid']=not trace['output_normalization']['changes']
                    ...
                    return result
                calls=turn['calls']
                if (final_only or trace['tool_requests_seen']>self.limits.tool_attempts):
