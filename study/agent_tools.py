...
from legacy.retrieval import search_notes, _terms, NOTES_DIR
...
def definitions(history_enabled=False):
    ...
    if history_enabled:
        specs.append(('get_review_history','查当前用户已确认收藏的相关记录；limit 只能为 1、2 或 3，最多返回 3 条。旧模型分析未经教师核对，不代表能力或掌握。',
                      {'topic':{'type':'string'},'limit':{'type':'integer','enum':[1,2,3]}},['topic','limit']))
