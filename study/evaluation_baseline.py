...
from study.agent_protocol import AgentError, parse_turn, validate_final, normalize_final
...
class FixedWorkflowService(StudyService):
    def __init__(self, config, key, *, scope, transport, output_mode='json_object'):
    ...
    def call(self, operation, instructions, context, image=None, work_image=None):
        ...
        trace = self.last_run = {'context': deepcopy(context), 'status': 'running', 'output_mode': self.output_mode,
        ...
        try:
            ...
            suffix = final_json_instructions(operation, context=context) if self.output_mode=='json_object' else ''
            payload = build_payload(self.config, instructions + BASELINE_PROMPT + suffix,
                                    {**context, 'fixed_reference_results': results})
            if self.output_mode=='strict_tool':
                payload = strict_final_payload(payload, operation, context=context)
            ...
            try:
                ...
                value = validate_final(candidate, trace['sources'], validate)
