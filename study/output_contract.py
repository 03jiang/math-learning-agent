...
from legacy.model_api import ModelAPIError
...
FINAL_FUNCTION = 'return_study_result'
...
def final_schema(operation, *, context):
    return obj({'result': output_schema(operation, context=context),
                'citations': array(obj({'source_id': string(), 'quote': string()}))})


def final_json_instructions(operation, *, context):
    """A/B 共用现有结构定义；这是生成提示，不替代返回后的严格校验。"""
    schema = final_schema(operation, context=context)
    return ('\n最终正文必须符合以下 JSON Schema。它只约束最终 result/citations，'
            '不改变工具回合。每个对象只能包含 properties 列出的字段，required 中的字段必须齐全。'
            '不要增加备注字段，即使值为空；补充说明写入已有的说明字段。'
            '输出前检查嵌套层级和字段名，只输出结果对象，不输出 Schema 本身。\n'
            + json.dumps(schema, ensure_ascii=False, separators=(',', ':')))


def strict_final_payload(payload, operation, *, context, read_tools=()):
    """复用 strict 模式承载最终对象；结果函数不执行，也不算资料查询。"""
    result = deepcopy(payload)
    result.pop('response_format', None)
    result['messages'][0]['content'] += (
        '\n本次启用严格返回格式。前述最终 JSON 对象仅放入 '
        + FINAL_FUNCTION + ' 的 arguments，不输出 content。它只返回 result/citations，'
        '不会读写文件或执行动作；可以直接返回，无需先查询资料。'
        '需要资料时先调用提供的只读查询工具，拿到结果再单独返回最终对象。'
        '不要在同一回合混合结果函数与查询工具；所有返回仍需本机数学和引用校验。')
    result['tools'] = deepcopy(list(read_tools))
    result['tools'].append({'type': 'function', 'function': {
        'name': FINAL_FUNCTION, 'description': 'Return the final study result and citations. Does not execute or save anything.',
        'parameters': final_schema(operation, context=context)}})
    for tool in result['tools']:
        tool['function']['strict'] = True
    result['tool_choice'] = ('required' if read_tools else
                            {'type': 'function', 'function': {'name': FINAL_FUNCTION}})
    return result
