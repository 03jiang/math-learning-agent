...
def perform(notebook, request_key, method, *args, **kwargs):
    ...
    if enabled():
        ...
        tutor=AgentStudyService(tutor.config,tutor.key,transport=tutor.transport,scope=scope(notebook,excluded),
                                audit_dir=audit(notebook).directory,output_mode=tutor.output_mode)
    try:
