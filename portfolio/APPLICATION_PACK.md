...
可补充的真实故障复盘：针对等价方法误判、有效澄清被误拒、局部步骤连带判错和仅答案推断过程，保留失败原文并修订提示词与证据检查；四条重复案例真实复测符合本次重点预期，两张合成图完成确认收藏与恢复。仍记录参考步骤配对问题，无教师评分，不声称稳定准确率或学习提升。完整证据范围见[B阶段记录](../docs/B_RETEST.md)。

当前可补充一条工程进展：设计并接入可选只读工具循环，让模型协议选择笔记或历史查询；程序执行参数、预算与来源检查，保留执行记录和用户确认。已用本机 HTTP 与页面测试验证边界，真实工具决策尚未验收。

评估设计可补充：构建30条自写用例、按15个题目家族划分20条开发与10条保留集，包含8条多轮交互；设计固定流程与有限工具循环的公平对照，逐回合记录输入资料、调用与失败，配套有证据要求的人工评分表。当前完成的是离线脚手架，真实批量与人工评分未完成，不能写成“提升准确率”或“已通过教师评估”。
...
原型用 Codex 辅助开发，提供无密钥演示和自动测试。已有真实文字订正和两张合成题图的收藏恢复记录，遇到模型误判和程序误拒后保留失败并做了复测。真实手机手写、模型选工具与学习效果还不能证明；下一步验证工具决策和对照策略，再做教师核对与学生任务观察。”
