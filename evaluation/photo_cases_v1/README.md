...
**2026-09-17 已运行其中两张合成图片的真实 OCR，人工评分仍为空。** p01_fraction_steps 与 q10_teacher_annotation 的题干、学生原作答和作答类型经助手对照参考及原图一致；尚未进行这两题的模型分析，其余 11 张没有本次真实结果。详见[运行记录摘要](../../docs/PHOTO_SMOKE.md)。

`manifest.json` 与 `score-template.json` 保留为测试集创建时的清单和空白模板，实际运行账本单独保存在本机，不将助手核对填成教师评分。两个真实拍摄场景仍为 `awaiting_image`，不能算作已覆盖。
