...
from model_api import (ApiTutor, HttpTransport, ModelAPIError, ModelConfig,
...
class ModelConfigTests(unittest.TestCase):
    ...
    def test_default_mock_never_uses_an_available_key(self):
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-private-key'}), patch('model_api.subprocess.run') as run:
    ...
    def test_invalid_config_cannot_enable_network_or_save(self):
        ...
        for values in cases:
            with self.subTest(values=values):
                ...
                with self.assertRaises(ValidationError), patch('model_api.subprocess.run') as run:
    ...
    def test_missing_and_invalid_keys_fail_before_request(self):
        ...
        for key in ['', 'a\nb', ' a', '密钥', 'x' * 4097]:
            with self.subTest(key_length=len(key)), self.assertRaises(ModelAPIError), patch('model_api.subprocess.run') as run:
                ApiTutor(config, key)
            run.assert_not_called()
    ...
    def test_check_command_never_launches_or_exposes_key(self):
        ...
        for key, expected in [('', 2), ('test-private-key', 0)]:
            ...
            result = subprocess.run([sys.executable, str(ROOT / 'run_app.py'), '--check', '--config', str(path)],
    ...
    def test_noninteractive_start_without_key_does_not_prompt_or_start_server(self):
        path = self.root / 'config.json'
        path.write_text(json.dumps(asdict(ModelConfig(mode='api', model='fixture-model'))))
        result = subprocess.run([sys.executable, str(ROOT / 'run_app.py'), '--config', str(path)],
...
class HttpModelTests(unittest.TestCase):
    ...
    def test_worker_start_failure_records_zero_attempts_and_no_key_in_arguments(self):
        with patch('model_api.subprocess.run', side_effect=OSError('private-error-sentinel')) as run:
            result = self.run_round()
    ...
    def test_real_attempt_counter_contract_with_injected_process_output_only(self):
        ...
        with patch('model_api.subprocess.run', return_value=subprocess.CompletedProcess([], 0, stdout, '')):
