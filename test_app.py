...
from model_api import ApiTutor, HttpTransport, ModelConfig, load_model_config
...
class StreamlitTests(unittest.TestCase):
    def setUp(self):
        ...
        config_path.write_text((APP.parent / 'model_config.example.json').read_text())
    ...
    def test_replay_reply_renders_validated_source_and_still_needs_confirmation(self):
        raw = (APP.parent / 'examples' / 'model_reply.json').read_text()
    ...
    def test_unsupported_method_claim_keeps_answer_check_but_hides_suggestion(self):
        value = json.loads((APP.parent / 'examples' / 'model_reply.json').read_text())
    ...
    def test_chat_next_button_accepts_once_and_keeps_reference_answers_collapsed(self):
        with LocalModelServer() as server:
            ...
            tutor = ApiTutor(load_model_config(APP.parent / 'model_config.deepseek.example.json'),
