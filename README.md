...
已完成一个真实文字订正保存与恢复闭环，以及两张合成图片的真实 OCR → 核对 → 分析 → 用户确认收藏 → 新进程恢复。修复后的四条真实复测均通过校验，助手复核确认本次等价解法、缺条件澄清、局部错步和仅答案边界符合预期；部分参考步骤配对仍不够贴切，人工评分为空。这是重复合成样本的故障复测，真实手机手写、真实工具选择和广泛教学质量仍待验证。详见[本次结果与限制](docs/B_RETEST.md)及[当前状态](docs/STATUS.md)。仓库保持私有。
...
**演示模式只回放一道人工作答样例，不执行 OCR、不调用模型，也不为任意新题目编造回复。** 数据保存在独立的 `data/demo-study/`。真实模式运行 `python -m study.launch`，在侧栏隐藏输入密钥；点击识图、分析或订正分析才发送当前题目内容，不自动重试。两张合成图流程已有真实记录，手机手写与广泛质量仍待验证，见 [验证状态](docs/STATUS.md)。
