...
class ConfirmationTests(unittest.TestCase):
    ...
    def test_expired_suggestion_rejected(self):
        before = self.seed()
        p = self.app.propose({"current_step": "确认两个分数对应同样大小的整体"})
        with patch("core.time.time", return_value=p.expires_at):
            with self.assertRaises(ValidationError):
                self.app.confirm(p)
        self.assertEqual(before, self.path.read_bytes())
    ...
    def test_save_failure_keeps_old_data_and_allows_retry(self):
        before = self.seed()
        p = self.app.propose({"current_step": "确认两个分数对应同样大小的整体"})
        for target in ["core.os.fsync", "core.os.replace"]:
    ...
    def test_restart_in_separate_python_process(self):
        ...
        result = subprocess.run(
            [sys.executable, "-c", "from core import LearningAssistant; from dataclasses import asdict; "
