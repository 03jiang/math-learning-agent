...
from legacy.curriculum import STEPS, MockTutor, check_answer, initial_snapshot
from legacy.http_test_support import LocalModelServer, chat_envelope
from legacy.model_api import ApiTutor, HttpTransport, load_model_config
...
class RevisionChatTests(unittest.TestCase):
    def setUp(self):
        ...
        config = load_model_config(Path(__file__).resolve().parents[1] / 'model_config.deepseek.example.json')
