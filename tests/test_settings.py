...
from legacy.core import (LEGACY_SETTING_FIELDS, SETTING_OPTIONS, LearningAssistant,
                  LearningSettings, ValidationError, decode_snapshot)
from legacy.curriculum import ANSWERS, TASKS, MockTutor, initial_snapshot
...
class SettingsTests(unittest.TestCase):
    ...
    def test_confirm_upgrades_legacy_once_and_real_new_process_restores_it(self):
        ...
        script = ('import json,sys; from dataclasses import asdict; from legacy.core import LearningAssistant; '
    ...
    def test_invalid_expired_or_stale_new_settings_cannot_write(self):
        ...
        with patch('legacy.core.time.time', return_value=expired.expires_at):
            with self.assertRaises(ValidationError):
    ...
    def test_failed_upgrade_keeps_legacy_file_and_proposal_retryable(self):
        ...
        before = self.path.read_bytes()
        proposal = self.assistant.propose(configuration_patch=ENABLED)
        with patch('legacy.core.os.replace', side_effect=OSError('simulated disk failure')):
