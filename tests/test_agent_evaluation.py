...
class PlanTests(unittest.TestCase):
    ...
    def test_strict_baseline_and_agent_share_final_schema_without_executing_result(self):
        from study.agent import AgentStudyService
        from tests.test_study_agent import strict_final
        case=load_cases()[0];turn=case['turns'][0]
        schemas=[]
        with LocalModelServer() as server:
            for arm in ('A','B'):
                context=model_context(case,turn,task_for_turn(None,case,turn),arm)
                envelope=handwritten_response(context,'analyze',case,arm,{'messages':[]})
                value=json.loads(envelope['choices'][0]['message']['content'])
                canonical=deepcopy(value['result'])
                value['result']['student_review']['answer_feedback_note']=''
                server.body=strict_final(value['result'],value['citations'])
                scope=seed_scope(self.root/arm,case,stamp())
                kwargs=dict(scope=scope,transport=PhotoTransport(server.chat_url),output_mode='strict_tool')
                tutor=(FixedWorkflowService(default_config(),'local-test-key',**kwargs) if arm=='A' else
                       AgentStudyService(default_config(),'local-test-key',audit_dir=self.root/'logs',**kwargs))
                self.assertEqual(canonical,tutor.call('analyze',ANALYSIS_PROMPT,context))
                self.assertFalse(tutor.last_run['raw_final_valid'])
                self.assertEqual(1,len(tutor.last_run['output_normalization']['changes']))
                self.assertEqual([],tutor.last_run['tool_calls'])
                schemas.append(server.requests[-1]['payload']['tools'][-1]['function']['parameters'])
                self.assertFalse(scope.history_dir.exists())
            self.assertEqual(schemas[0],schemas[1]);self.assertEqual(2,len(server.requests))
