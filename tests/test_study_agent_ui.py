...
from tests import test_study as fixtures
from tests.test_study_agent import final, strict_final, note_call, calls, history_entry
...
class AgentAppTests(unittest.TestCase):
    ...
    def test_existing_strict_output_setting_is_preserved_when_agent_enabled(self):
        self.prepare();self.server.body=strict_final()
        with patch('study.ui.service',side_effect=lambda:StudyService(default_config(),
                'local-test-key',PhotoTransport(self.server.chat_url),output_mode='strict_tool')):
            self.click('分析这道题')
        self.assertEqual('success',self.latest()['status']);self.assertEqual('strict_tool',self.latest()['output_mode'])
        self.assertTrue(all(t['function']['strict'] for t in self.server.requests[0]['payload']['tools']))
