...
from legacy.curriculum import TASKS
from legacy.evaluate_teaching import main, run_batch
from legacy.evaluation_cases import load_cases
from legacy.evaluation_scoring import FIELDS, METRICS, summarize_scores
from legacy.http_test_support import chat_envelope
...
class EvaluationTests(unittest.TestCase):
    ...
    def test_preview_has_96_requests_zero_http_even_with_key_and_empty_scores(self):
        with patch.dict(os.environ, {'DEEPSEEK_API_KEY': 'SHOULD_NOT_READ'}), patch('legacy.model_api.HttpTransport.send') as send:
    ...
    def test_revision_suite_freezes_explicit_buttons_and_partial_reading(self):
        with patch('legacy.model_api.HttpTransport.send') as send:
    ...
    def test_resume_sends_only_missing_rows_and_preserves_manual_scores(self):
        ...
        with patch('legacy.model_api.HttpTransport.send', side_effect=AssertionError('repeat call')):
    ...
    def test_existing_output_and_changed_freeze_are_rejected_without_calls(self):
        ...
        with self.assertRaises(FileExistsError):
            run_batch(self.output, suite='smoke')
        with patch('legacy.evaluate_teaching.frozen_sources', return_value={'changed.py': 'new'}), self.assertRaises(ValidationError):
    ...
    def test_concurrent_resume_is_refused_before_http(self):
        ...
        with (self.output / '.run.lock').open('a') as lock:
            ...
            with patch('legacy.model_api.HttpTransport.send') as send, self.assertRaises(ValidationError):
                run_batch(self.output, mode='local_http_test', suite='smoke', resume=True)
            send.assert_not_called()
