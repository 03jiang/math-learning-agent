...
from core import LearningAssistant, ValidationError
from http_test_support import LocalModelServer, successful_envelope
from model_api import ModelAPIError, ModelConfig
from probe_model import CASE, ROOT, main, probe
...
class ProbeTests(unittest.TestCase):
    ...
    def test_default_preview_cannot_start_http_even_with_live_environment(self):
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-secret-sentinel',
                ...
                patch('probe_model.ApiTutor') as tutor, patch('probe_model.LocalModelServer') as server:
    ...
    def test_existing_directory_file_or_symlink_prevents_all_requests(self):
        ...
        for target in (self.output, marker, link):
            with self.subTest(target=target.name), patch('probe_model.ApiTutor') as tutor, \
                    patch('probe_model.LocalModelServer') as server, self.assertRaises(ValidationError):
    ...
    def test_live_requires_valid_config_and_key_before_artifacts(self):
        for config, key in ((None, ''), (ModelConfig(), 'dummy'),
            ...
            with self.subTest(config=config), self.assertRaises((ValidationError, ModelAPIError)), \
                    patch('model_api.subprocess.run') as run:
    ...
    def test_actual_http_once_matches_preview_and_never_confirms(self):
        ...
        with patch('probe_model.LocalModelServer', return_value=server), \
    ...
    def test_interrupt_retains_in_progress_report_instead_of_success(self):
        with patch('probe_model.Workspace.run', side_effect=KeyboardInterrupt), self.assertRaises(KeyboardInterrupt):
            probe(self.output, mode='local_http_test')
    ...
    def test_cli_preview_and_noninteractive_live_missing_key(self):
        ...
        command = [sys.executable, str(ROOT / 'probe_model.py'), '--output', str(self.output)]
    ...
    def test_cli_failure_exit_code_and_config_option_validation(self):
        ...
        with patch('probe_model.load_model_config') as load:
